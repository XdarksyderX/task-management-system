        ...
        if cls._publisher is None:
            ...
            try:
                if publisher_type == "kafka":
                    ...
                    if (
                        not hasattr(cls._publisher, "producer")
                        or cls._publisher.producer is None
                    ):
                        logger.warning(
                            "Kafka producer not available, falling back to memory publisher"
                        )
                ...
                logger.error(
                    f"Failed to initialize {publisher_type } publisher: {str (e )}"
                )
