...
def publish_task_event(
    ...
    try:
        ...
        success = publisher.publish(
            topic=TASK_EVENTS_TOPIC, event=payload, key=message_key
        )
...
def publish_task_updated(
    user_id: int, task_id: int, title: str, changes: Dict[str, Any]
):
    """Publishes task update event"""
    data = {"task_id": task_id, "title": title, "changes": changes, "action": "update"}
...
def publish_task_deleted(user_id: int, task_id: int, title: str):
    ...
    data = {"task_id": task_id, "title": title, "action": "delete"}
    return publish_task_event(TaskEventType.TASK_DELETED, user_id, data)


def publish_task_status_changed(
    user_id: int, task_id: int, title: str, old_status: str, new_status: str
):
...
def publish_task_completed(user_id: int, task_id: int, title: str):
    ...
    data = {"task_id": task_id, "title": title, "action": "complete"}
    return publish_task_event(TaskEventType.TASK_COMPLETED, user_id, data)


def publish_task_assigned(
    user_id: int,
    task_id: int,
    title: str,
    assigned_to_id: int,
...
def publish_task_priority_changed(
    user_id: int, task_id: int, title: str, old_priority: str, new_priority: str
):
...
def publish_task_archived(user_id: int, task_id: int, title: str):
    ...
    data = {"task_id": task_id, "title": title, "action": "archive"}
...
def publish_tag_created(user_id: int, tag_id: int, tag_name: str, color: str = None):
    ...
    data = {"tag_id": tag_id, "tag_name": tag_name, "color": color, "action": "create"}
    return publish_task_event(TaskEventType.TAG_CREATED, user_id, data)


def publish_tag_updated(
    user_id: int, tag_id: int, tag_name: str, changes: Dict[str, Any]
):
...
def publish_tag_deleted(user_id: int, tag_id: int, tag_name: str):
    ...
    data = {"tag_id": tag_id, "tag_name": tag_name, "action": "delete"}
    return publish_task_event(TaskEventType.TAG_DELETED, user_id, data)


def publish_task_tag_added(
    user_id: int, task_id: int, task_title: str, tag_id: int, tag_name: str
):
...
def publish_template_updated(
    user_id: int, template_id: int, template_title: str, changes: Dict[str, Any]
):
...
def publish_task_created_from_template(
    user_id: int, task_id: int, task_title: str, template_id: int, template_title: str
):
