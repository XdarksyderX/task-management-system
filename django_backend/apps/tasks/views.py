...
def task_list(request):
    ...
    user_tasks = (
        Task.objects.filter(assigned_to=request.user, is_archived=False)
    ...
    if request.method == "POST":
        ...
        if action == "mark_done" and task_id:
            try:
                ...
                publish_task_status_changed(
                    request.user.id, task.id, task.title, old_status, TaskStatus.DONE
                )
...
def task_detail(request, pk):
    ...
    task = get_object_or_404(
        Task.objects.select_related(
            "created_by", "assigned_team", "parent_task"
...
def template_list(request):
    ...
    templates = TaskTemplate.objects.filter(created_by=request.user).order_by("name")
    ...
    if request.method == "POST":
        ...
        if name:
            ...
            template = TaskTemplate.objects.create(
                name=name, template=template_data, created_by=request.user
            )

            publish_template_created(
                request.user.id, template.id, template.name, description, priority
            )
