...
from apps.tasks.models import Task, TaskHistory, TaskAction, TaskStatus
...
def generate_daily_summary():
    ...
    users = (
        ...
        .annotate(
            t_cnt=Count("tasks_assigned", distinct=True)
            + Count("tasks_created", distinct=True)
        )
        .filter(t_cnt__gt=0)
    )
...
def send_websocket_comment(task_id, comment_id, event_type):
    ...
    try:
        ...
        async_to_sync(channel_layer.group_send)(
            room_group_name,
            {"type": f'comment_{event_type .split ("_")[1 ]}', "comment": comment_data},
        )
