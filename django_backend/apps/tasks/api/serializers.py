...
from apps.tasks.models import (
    Tag, Task, Comment, TaskAssignment, TaskHistory, TaskTemplate,
...
class TaskSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            "estimated_hours","actual_hours","created_by","assigned_team",
            "assigned_to","tags","parent_task","metadata","is_archived",
            "created_at","updated_at"
        ]
        read_only_fields = ["id","created_by","is_archived","created_at","updated_at"]
    ...
    def create(self, validated_data):
        ...
        if tags:
            task.tags.set(tags)
        TaskHistory.objects.create(task=task, user=request.user, action=TaskAction.CREATED)
        return task
    ...
    def update(self, instance, validated_data):
        ...
        if old_status != instance.status:
            ...
            TaskHistory.objects.create(task=instance, user=request.user, action=TaskAction.UPDATED)
        return instance
...
class CommentSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        TaskHistory.objects.create(
            task=obj.task, user=request.user,
            action=TaskAction.COMMENT_ADDED, metadata={"comment_id": obj.id}
        )
        return obj
