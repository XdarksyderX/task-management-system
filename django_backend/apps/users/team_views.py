...
def team_list(request):
        ...
        if action == "delete_team":
            ...
            if team_id:
                try:
                    ...
                    team.delete()
                    messages.success(
                        request, f'Team "{team_name }" deleted successfully.'
                    )
            ...
            if team_id:
                try:
                    ...
                    if request.user in team.members.all():
                        ...
                        messages.success(
                            request, f'You have left the team "{team .name }".'
                        )
                    else:
    ...
    context = {"teams": user_teams, "user": request.user}
        ...
        if action == "add_member" and team.created_by == request.user:
            ...
            if user_id:
                try:
                    ...
                    if (
                        user_to_add not in team.members.all()
                        and user_to_add != team.created_by
                    ):
                        team.members.add(user_to_add)
                        messages.success(
                            request, f"User {user_to_add .username } added to team."
                        )
                    else:
                        messages.warning(
                            request, "User is already a member of this team."
                        )
            ...
            if user_id:
                try:
                    ...
                    messages.success(
                        request, f"User {user_to_remove .username } removed from team."
                    )
                except User.DoesNotExist:
                    messages.error(request, "User not found.")
            ...
            team.delete()
            messages.success(
                request, f'Team "{team_name }" has been deleted successfully.'
            )
            return redirect("users:team_list")
