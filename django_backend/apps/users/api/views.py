...
class CustomTokenObtainPairView(TokenObtainPairView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if response.status_code == 200:
            ...
            next_url = request.data.get("next") or request.GET.get(
                "next", "/dashboard/"
            )
...
class TeamViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return (
            Team.objects.filter(models.Q(created_by=user) | models.Q(members=user))
            .distinct()
            .order_by("-created_at")
        )
    ...
    def perform_create(self, serializer):
        ...
        publish_team_created(self.request.user.id, team.id, team.name, team.description)
    ...
    def leave(self, request, pk=None):
        ...
        team.remove_member(user)
        return Response(
            {"message": f"You have left team {team .name }"}, status=status.HTTP_200_OK
        )
