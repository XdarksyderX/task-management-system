...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--users", type=int, default=25, help="Number of users to create"
        )
        parser.add_argument(
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.SUCCESS(
                ...
                + "\n".join(
                    [f"- {user .username } (password123)" for user in users[1:6]]
                )
            )
        )
            ...
            if random.random() > 0.4:
                ...
                for _ in range(num_comments):
                    ...
                    Comment.objects.create(
                        task=task, author=random.choice(users), body=comment_body
                    )
