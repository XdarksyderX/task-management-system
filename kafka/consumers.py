...
TOPICS = [t.strip() for t in os.getenv("KAFKA_TOPICS", "user-activities,task-events,analytics-events,system-notifications").split(",") if t.strip()]
...
while not _shutdown:
	...
	if ROLE == "notifications":
		rd.lpush("notifications", json.dumps({"topic": msg.topic(), "event_type": et, "user_id": uid, "data": data}))
		...
		with pg.cursor() as cur:
			cur.execute("insert into activity_feed(user_id,event_type,entity,entity_id,payload) values (%s,%s,%s,%s,%s)", (uid, et, ent, ent_id, json.dumps(v)))
		...
		with pg.cursor() as cur:
			cur.execute("insert into audit_logs(user_id,event_type,payload) values (%s,%s,%s)", (uid, et, json.dumps(v)))
